...
class QLearningAgent:
    ...
    def __init__(self, frames_shape, action_space):
    ...
    def _build_image_model(self):
        model = Sequential()
        model.add(Conv2D(32, (8, 8), strides=4, activation='relu', input_shape=self.frames_shape, data_format='channels_first'))
    ...
    def replay(self, replay_length):
        ...
        for experience in replay_batch:
            if not experience.done:
                target = experience.reward + self.discount * np.amax(self.target.predict(experience.next_state)[0])
            else:
                target = experience.reward

            prediction = self.model.predict(experience.state)[0]
            prediction[experience.action] = target

            xs.append(experience.state[0, :, :, :])    # [0] removes  that dimension from the 4d array, taking it to 3d
            ys.append(prediction)

        # model training
        self.model.fit(np.array(xs), np.array(ys), batch_size=32, epochs=1)
